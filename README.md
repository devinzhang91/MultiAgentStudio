...
每个员工可以配置独立的 OpenClaw 连接参数：

| 配置项 | 说明 | 示例 |
|--------|------|------|
| `base_url` | OpenClaw Gateway URL | `wss://gateway.openclaw.com` |
| `token` | 认证令牌 | `oc_token_xxx` |
| `session_key` | 会话标识 | `alice-session` |
| `timeout` | 请求超时(秒) | `120` |
...
**员工管理:**
- **添加员工**: 按 `a` 或点击"➕ 添加员工"按钮
  - 输入名称、角色
  - 配置 OpenClaw 连接参数 (base_url, token, session_key, timeout)
  - 选择是否启用连接
- **查看属性**: 选中员工卡片，按 `Space` 或点击卡片
  - 查看所有配置信息
  - 点击"🔄 重启连接"重新连接 OpenClaw
  - 点击"🗑️ 删除员工"删除该员工
- **删除员工**: 在属性界面点击删除按钮

**对话操作:**
- **进入对话**: 点击员工卡片 或选中后按 `Enter`
- **发送消息**: 输入内容后按 `Enter`
- **返回工作室**: 按 `Esc` 或点击返回按钮
...
✅ **支持环境:**
- 本地终端（Windows Terminal、iTerm2、GNOME Terminal 等）
- **SSH 远程连接**（无需 GUI）
- **无图形界面的服务器**（只要有终端即可）
- Docker 容器内部
- VS Code 集成终端
...
⚠️ **要求:**
- Python 3.9+
- 支持 Unicode 和 emoji 的终端
- 终端大小建议：80x24 或更大

💡 **提示:** 在 SSH 或服务器上运行时，鼠标支持取决于终端模拟器的能力。大多数现代终端（如 Windows Terminal、iTerm2、GNOME Terminal）都支持鼠标。
